...
Updating Interceptor itself from an agent: `interceptor update`, then `interceptor macos tree --app interceptor-bridge --filter all`, then `act` the "Install Update" button and, once Sparkle has downloaded, the "Install and Relaunch" button. The last step raises a macOS administrator prompt that a person must answer (package installs always do); do not try to drive that dialog.
