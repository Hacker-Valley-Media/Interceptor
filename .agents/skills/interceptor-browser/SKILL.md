...
- A page that will not render while hidden (empty grid, "Loading" that never resolves, an app that pauses on `document.hidden`): run `interceptor tab keepalive <id>`, then `read` again. The page reads as visible and its animation frames run; the user's view is untouched. Do not `tab switch` for rendering. `tab switch` is only for a trusted click that needs the tab on screen, and afterwards `tab switch <prior>` back to the tab that was showing is allowed even though it is unmanaged. Limits: IntersectionObserver and ResizeObserver callbacks still wait for a real paint, and background timers stay at Chromium's rate (about once per second after 10 s).
