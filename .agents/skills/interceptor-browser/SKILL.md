...
- Claude Code shells get a per-session group automatically (derived from the session id, labeled `s-<hash8>`), so bare commands already reuse one tab per session and the idle sweeper has a unit to reap — you do not need to pick a label for solo work. Derived scope is soft: an empty group falls back to the active tab, and explicit `--tab` targets are not membership-gated against it. **Parallel sibling lanes share one session id**, so lanes that run Interceptor concurrently MUST each pass an explicit `--group lane-<n>` — a shared derived group would let one lane's `open` navigate a tab another lane is reading. An explicit `--group <label>` (or `INTERCEPTOR_GROUP`) buys hard isolation: resolution never leaves the group and cross-group targets are rejected. `--no-group` (or `INTERCEPTOR_GROUP=` empty) opts a call out entirely. `interceptor group list` shows what's running, including your derived group's actual label.
- Close your group with `interceptor group close <label>` the moment the job is done (didn't choose a label? `interceptor group list` shows your derived group's) — tabs you leave open are tabs the user has to close by hand. The extension auto-closes idle groups (default: after 10 minutes without a command; configurable in the extension popup), but that is the crash-safety floor, not a substitute for cleaning up.
