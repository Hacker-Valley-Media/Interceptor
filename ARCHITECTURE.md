...
- **On Windows the equivalent is `scripts/install.ps1`** (`pwsh -File scripts\install.ps1 -Browser edge -ProfileName Default`). It registers and launches only — `bash scripts/build.sh --target=windows` has to have run first, and the script hard-errors naming that command if the daemon binary or extension bundle is missing. The registration mechanism also differs: instead of dropping a manifest file into a per-browser directory, it generates `daemon/.generated/com.interceptor.host.json` and points `HKCU:\Software\{Google\Chrome,BraveSoftware\Brave-Browser,Microsoft\Edge}\NativeMessagingHosts\com.interceptor.host` at it — **one key per browser**, so a browser that was never passed to the script has no host registered. It then preflights `extensions.ui.developer_mode` in the target profile's `Preferences` JSON (Chromium silently drops `--load-extension` when Dev mode is off, producing a dormant install that reports success), launches the browser, and probes `interceptor status --verbose` for `extension: reachable` for up to 8s. `-Full` is rejected: the Swift bridge is macOS-only, so `mode: browser-only` is the only reachable Windows mode. Packaged Windows installs go through `scripts/installer/interceptor.iss` (Inno Setup) instead, which targets `%LOCALAPPDATA%\Programs\Interceptor` rather than the repo tree.
...
1. **Native renderer, no library.** `extension/src/content/dom-screenshot.ts` does the whole render inside the target tab using platform primitives only — `getComputedStyle`, `cloneNode`, `XMLSerializer`, `<svg><foreignObject>`, `Image`/`decode()`, `Canvas`, `FileReader`, and `fetch`. There is no injected bundle and no vendored dependency, so pages that never get screenshotted pay no cost and there is no on-demand `chrome.scripting.executeScript` step. It matters that no step awaits `requestAnimationFrame`: rAF is suspended on hidden tabs, `decode()` is not, which is what lets the path run unthrottled on a backgrounded tab.
...
3. **Render.** `resolveTarget` picks the node by mode (`full`/`region` → `document.documentElement`, `element` → refRegistry lookup, `selector` → `querySelector`). `buildStyledClone` then walks the tree, inlining each element's computed style onto the clone and dropping non-visual tags (`script`, `style`, `link`, `meta`, `template`, `iframe`, `object`, `embed`). Three cases get special handling: `<canvas>` is snapshotted via `toDataURL()` into an `<img>`, falling through to a blank structural clone if the canvas is tainted; inline `<svg>` is deep-cloned wholesale with only its root style inlined, since descendants inherit `color` and per-descendant inlining is what made SVG-heavy pages (Wikipedia ships ~2.6k inline icons) time out; and `<img>`/`background-image` URLs are collected, fetched once in parallel with `{ mode: "cors", cache: "force-cache" }`, and embedded as data URLs, with any failure substituting a 1×1 transparent PNG so the canvas never taints and `toDataURL()` never throws. The clone is serialized into a `foreignObject` SVG, loaded through `Image` + `decode()`, and drawn to a canvas. `checkRasterizeOutput` then rejects a near-empty payload: a canvas past the browser's dimension/area limit does not throw, it silently fails to allocate and returns `"data:,"`, which would otherwise be reported as a successful capture of size 0.
4. **Region crop at rasterize time.** For `--region` the canvas is allocated at the crop size and the decoded SVG is drawn into it at an offset, so the inter-process message back to the SW carries only the cropped result instead of a multi-MB full-page payload **and** pages whose full-size canvas would exceed the browser's canvas limits still capture successfully.
...
- CLI-first Brave install path through `scripts/install.sh --brave --profile <profile>` (Windows: `scripts/install.ps1`, registry-based native-messaging registration)
