...
- **CLI** is a Bun-bundled standalone binary. It parses args, sends an action over `<runtimeDir>/interceptor.sock` to the daemon, and prints the response. `runtimeDir` is the current user's temporary directory (`$TMPDIR`, else `getconf DARWIN_USER_TEMP_DIR`; `$XDG_RUNTIME_DIR` on Linux; `%TEMP%` on Windows), so two logged-in accounts never share a socket, pid, lock, log, or event file.
- **Daemon** is a per-user singleton whose token is that user's WebSocket port: `derivePorts(uid)` in `shared/platform.ts` maps uid 501 to the published 19221/19222 pair, uid 504 to 19227/19228, and so on across 500 slots (env overrides `INTERCEPTOR_WS_PORT` / `INTERCEPTOR_IPC_PORT` win; Windows has no uid and stays on slot 0). The bridge, the runtime agent, and the Safari appex compute the same mapping from `getuid()`. `<runtimeDir>/interceptor.pid`, `.sock`, and `.lock` only describe the port owner, and the owner restores them if they go missing (see *Singleton ownership and self-heal* below). Spawned automatically by Chrome via native messaging, *or* started by the CLI on demand. It bridges CLI ⇄ extension ⇄ bridge, owns event persistence, and tracks per-session monitor artifacts.
...
└─────────────────────────────────┘
                ...
                ▼
┌─────────────────────────────────┐
│  Global rolling event log       │   <runtimeDir>/interceptor-events.jsonl
│  daemon emitEvent → appendFile  │   useful for `monitor tail`, rotates
└─────────────────────────────────┘
                │ daemon side-write per sid
                ▼
┌─────────────────────────────────────────────────────┐
│  Per-session artifact directory                      │   <runtimeDir>/interceptor-monitor-sessions/<sid>/
...
### Native selection and browser field masking

`handleSelectOption` in `extension/src/content/actions/type.ts` validates native selects before mutation: exact option value first, otherwise one exact label, with disabled controls/options/optgroups and ambiguous or missing matches refused. It sets one selected option, dispatches input then change, and checks that synchronous handlers and microtasks retained the option. Native `handleInputText` routes through the same handler; `handleFindAndType` propagates its failure. Later asynchronous application changes still require a read.
...
Some pages return an empty accessibility tree while their DOM is fully queryable, which used to make them visible to `query` but unclickable. Two complementary lanes close that gap. `interceptor click --selector "<css>" [--nth N]` (`click_selector` action) clicks the Nth match of a shadow-piercing selector query directly, with honest errors carrying the match count for zero/out-of-range/invalid selectors. The normalizer accepts `--selector`/`--nth` for `click` only; every other action verb rejects them with a `query "<css>"` → `<verb> e<ref>` hint, because a value flag the parser does not implement would otherwise be swallowed as the element target. And `query` assigns every returned element a ref via `getOrAssignRef`, the same element-keyed registry the a11y tree uses, so DOM-discovered elements are actionable by every ref verb (`click e<N>`, `type e<N>`, …). Selector clicks report the clicked element's ref both in the message and as a structured `refId`, which the background router uses to join the synthetic→`os_click` auto-escalation lane on "no DOM change", resolving screen coordinates from the ref exactly like a ref-targeted click. `parseElementTarget` itself guards missing targets with a usage error (it previously threw), and malformed `--selector`/`--nth` values are rejected at parse time rather than falling through as bogus refs.

**Shadow-piercing selectors (`content/deep-query.ts`).** `document.querySelectorAll` does not descend into shadow roots, so a web component's only button was invisible to every CSS-selector verb while `read` and `find`, which walk with `getShadowRoot`, could see it. `queryAllDeep` visits the light DOM first and then every shadow root beneath it, breadth-first and iteratively with a 2000-root cap, so light-DOM matches keep their `querySelectorAll` order (and `--nth` its meaning) and shadow matches are appended after them; `queryOneDeep` short-circuits on the light DOM. Closed roots are reached through `chrome.dom.openOrClosedShadowRoot` where the browser offers it (Chrome 88 and later, Safari 26). Every selector verb resolves through these two functions: `query`, `exists`, `count`, `click --selector`, `wait`, `table`, `attr`, `style`, `rect`, and `screenshot --selector`. `waitForElement` polls every 250 ms as well as observing, because a `MutationObserver` on the light DOM never reports mutations inside a shadow tree. The unit cases run in a child process (`deep-query.check.ts`, spawned by `deep-query.test.ts`) because bun's `mock.module` is process-wide and several suites mock `element-discovery` with a null `getShadowRoot`.
...
**Render keep-alive and the way back.** Chromium sends no compositor frames to a hidden tab (`SetCompositorVisible(false)` in Blink's `WidgetBase`), so `requestAnimationFrame` never fires there and the page reads `document.hidden === true`; apps that gate rendering on either never draw, which is why agents used to `tab switch` a tab into view just to read it. `interceptor tab keepalive <id>` flips a symbol-keyed state that [`extension/src/inject-keepalive.ts`](extension/src/inject-keepalive.ts) installs inert on every page from the MAIN-world `document_start` script: while on and the tab is really hidden, `visibilityState`/`hidden`/`hasFocus()` read visible, a `visibilitychange` fires, and queued rAF callbacks drain once per tick of a Blob-worker `setInterval` (about 60 per second; each callback runs once, from whichever of the native frame or the ticker comes first, and a callback that re-queues is picked up by the next tick, never by a re-entrant drain). The worker is the point: Chromium aligns a hidden page's own timers to one wake-up per second, so a `setTimeout` drain gives one frame per second; that is the fallback, taken once, on a page whose CSP forbids `blob:` workers. The background keeps a one-time flag per tab ([`extension/src/background/tab-keepalive.ts`](extension/src/background/tab-keepalive.ts)) and re-applies it at `webNavigation.onCommitted`; it plays no audio and touches none of the idle-sweep guards, so a forgotten keep-alive tab is still swept. What it cannot do, stated in the CLI result: IntersectionObserver/ResizeObserver callbacks need a real lifecycle update. `chrome.tabCapture`, which would make Chromium itself treat the tab as visible, is not an option: `getMediaStreamId` refuses unless the user invoked the extension on that tab. When a trusted click does force a `tab switch`, the handler records the tab the window was showing together with the switching lane's group, only when that tab is unmanaged ([`extension/src/background/switch-back.ts`](extension/src/background/switch-back.ts)), and the dispatcher lets `tab switch <prior>` from that same lane through the managed-group gate once, without persisting the user's tab as an auto-target. `window new` sends `focused: false` unless `--activate` (Chromium's own default is focused), and `monitor export --plan` writes a recorded focus switch as a comment rather than a `tab switch` command.

Trusted credential typing also passes sensitivity through the focus request. `handleFocus` verifies focus before marking the target sensitive, and the background handler refuses delivery after a failed focus result. Delivery into the already-focused field preserves focus and marks the deepest active shadow descendant. `daemon/os-input.ts` assigns explicit modifier flags to every text event, including zero for lowercase, digits, Unicode, and Shift release, so the events do not inherit unrelated modifiers.
...
`extension/src/background/capabilities/evaluate.ts` now treats page CSP as a first-class runtime concern. On a `MAIN`-world eval failure that matches a CSP/`unsafe-eval` pattern, it installs a tab-scoped **session** `declarativeNetRequest` rule that strips `content-security-policy` and `content-security-policy-report-only`, reloads the tab, then retries once. This is the behavior proven against OpenStreetMap during live validation. The reload is disclosed twice: in `data.cspBypassApplied` and in a `warning` the CLI prints on both the recovered and the still-failed result, for `eval` and for `save` (the byte sink shares `runWithCspStripBypass` and keeps the warning through `prepareByteSource`). `interceptor eval --no-reload` sets `noCspReload`, the same field task verification uses, so a page whose state matters gets the CSP error instead of a reload.
...
[`extension/src/background/tab-lifecycle.ts`](extension/src/background/tab-lifecycle.ts) enforces cleanup so long automation runs stop accumulating tabs. Three knobs live under one `chrome.storage` key (`tabLifecycle`), resolved `managed > local > default` exactly like the brand identity (defaults: `reuse: true`, `idleCloseMinutes: 10`, `closeGroupWhenDone: false`); the extension popup is the only writer of the `local` tier. The `managed` tier is declared in [`extension/managed-schema.json`](extension/managed-schema.json) (`tabLifecycle` and `brandTabGroup`, named by `storage.managed_schema` in the Chrome manifest only), because Chrome publishes nothing through `storage.managed` without a schema and refuses to load an extension whose schema is invalid. When a managed value is present the popup shows it, disables those controls, and says the setting is managed.
...
- **Idle sweep** — tab-touching dispatches stamp `groupLastSeen:<label>` in `storage.session`, with the same lifetime as the group ids it describes. Metadata polls such as `status` and `group_list` do not count as tab activity and do not keep a group alive. A `chrome.alarms` tick (1 min, existence-checked on every SW startup because alarms may not survive browser restarts) closes any managed group idle past the configured minutes. Guards, in order: only managed groups are ever touched; never the focused window's active tab (unknown focus protects every active tab); never pinned; never audible; never a window's last tab (`selectSweepCandidates`, pure and unit-tested); and never a tab with unsaved user state. `chrome.tabs.remove` was proven live to bypass `beforeunload` entirely (handler never runs, no dialog), so the sweep injects a dirty-state check (form deltas vs. defaults, `window.onbeforeunload` registered) and keeps dirty tabs for the agent's own `group close`. Every tick first re-adopts named groups the registry lost (`readoptNamedGroups`, exact `<brand>-<label>` title match, shared with `group_list`): a group moved to another window, re-created by the browser's own restore, or restored after a restart comes back under a new id after `tabGroups.onRemoved` dropped its entry, and would otherwise never be swept. Its existing stamp keeps counting. Missing stamps grace-stamp instead of closing; survivors re-stamp so a guard-vetoed group isn't re-scanned every tick. Sweeps log to `storage.local` (`tabLifecycleSweepLog`, capped) and the SW console; swept tabs remain restorable via `sessions restore` / ⌘⇧T.
- **Full-group purge (`closeGroupWhenDone`, opt-in)**: the guarded sweep is deliberately conservative, and in agent use that conservatism leaves groups standing. A tab an agent typed into reads as dirty on every pass (and an all-dirty pass re-stamps the idle clock), and G5 never closes the last tab of a dedicated Interceptor window, so the group outlives the session it belonged to. With the popup toggle on, an idle group is not swept but **deleted**: `planGroupPurge` returns every tab id in the group, G5 and the dirty check are skipped (a timeout-is-dirty verdict on a wedged page would otherwise veto the purge), an active tab goes too unless a person is on it, and the group object disappears with its last tab. G3 has nothing to protect in either mode: Chrome removes a tab from its group the moment it is pinned (`groupId` becomes -1), so a pinned tab is never part of a managed group. Three properties are kept on purpose. (1) It cannot fire without `idleCloseMinutes > 0`: idle is the only "done" signal the extension has, since the CLI is one process per command. (2) It waits for a person. The idle stamp only sees CLI commands, so someone who took over an agent tab looks idle; while the group holds the active tab of the OS-focused window or an audible tab, the purge is deferred for that tick **without** re-stamping and lands on the first tick where neither holds. Unknown focus (the browser is not frontmost, the overnight-run case) does not defer. (3) It will not take the browser down with the group: `chrome.tabs.remove` on a window's last tab closes the window, and on Windows/Linux the last window closing quits the browser (service worker, daemon connection, and every other lane with it), so when the purge would empty the profile's normal windows an ungrouped blank survivor tab is created first, in the group's own window. The normal-window tab count is re-read per group, because an earlier purge in the same tick makes a cached count stale-high in exactly the direction that would skip the survivor; an unreadable count is treated as 0 for the same reason, and a survivor that cannot be created skips the group. A purged group's stamp is removed rather than refreshed, so the next group minted under that label starts from a grace stamp instead of looking instantly idle.
- **Observability** — the extension `status` action returns the resolved policy + source tier (`tabLifecycle: { reuse, idleCloseMinutes, closeGroupWhenDone, source }`), rendered by `interceptor status --verbose`. Sweep log entries carry `mode: "guarded" | "purge"`.
- **Restore semantics** — Chrome's session store records tab/group closes inside a surviving window as individual closed-tab entries (each remembers its `groupId`, so restoring them reassembles the group), while a whole-window close is a single restorable `window` entry. `sessions restore` therefore requires an explicit sessionId — the no-arg "most recently closed" form is refused, since it can grab the user's own window. `chrome.sessions.restore` always makes the restored tab the active tab of its window, so it sits behind `sessions restore <id> --activate`. The default looks the entry up in `getRecentlyClosed()` and sends each URL through `tab_create`: background tabs, homed and grouped like any other agent tab, with the caller group's idle stamp refreshed. That path gives up history and form state and leaves the entry in the recently closed list; the result says so.
...
- **WebSocket** (`ws://127.0.0.1:<wsPort>`, loopback only) — fallback / preferred for action requests. The extension dials the primary account's 19222 until the native host's `pong` reports this user's `wsPort` (`extension/src/background/ws-endpoint.ts`); a host without native messaging keeps dialing 19222, so on a multi-account Mac it attaches to the primary account's daemon until `interceptor init` or the installer writes the per-user host manifest
...
- **Safari native relay** — Safari's service worker long-polls its containing appex with `runtime.sendNativeMessage`; the appex owns a `URLSessionWebSocketTask` to `127.0.0.1:<wsPort>` (computed from `getuid()`) and relays the unchanged command/response envelope
...
- **Only one live instance.** The daemon's bridge-recovery ladder ([`daemon/bridge-recovery.ts`](daemon/bridge-recovery.ts)) used to escalate from `launchctl kickstart -k` to `open -gj <bundle>` after 1.5 s, but launchd throttles respawns for about 10 s, so the escalation routinely launched a second, unsupervised bridge that survived the next install. With a LaunchAgent on disk the ladder now emits only supervised actions: `launchctl bootstrap` when the agent is not loaded, then a plain `kickstart` (no `-k`, which would kill a start already in flight); the `open`/bare-binary actions remain for source checkouts without a LaunchAgent. Before any of that, the ladder asks launchd whether the account has a GUI domain at all (`shared/gui-session.ts`: `launchctl print gui/<uid>` exits 125 "Domain does not support specified action" when it is absent and something has run as that uid since boot, 112 "Bad request." when nothing has; the service print `gui/<uid>/com.interceptor.bridge` exits 113 "Bad request." when the domain exists but the service is not loaded). An ssh session or a service account has no domain, so the ladder emits no actions and both the daemon's error and the `interceptor status` hint say that the account has no GUI login and who owns the screen, instead of the "postinstall bootstrap failed" hint that used to fire for that case (one account over ssh while another account held the console). At process entry, `main.swift` holds `interceptor-bridge-instance.lock` for its lifetime, before PID or socket mutation. Package postinstall owns the supervised restart, while any additional Sparkle relaunch exits at the lock without touching the running bridge.
...
- **Path A (`interceptor cdp`)** — the daemon opens an **outbound** CDP WebSocket
  ...
  hardened Slack/Claude). `cdp launch` does that relaunch with `open -g`, then
  watches the frontmost app for a few seconds and puts the previous one back
  with `open -b` when the relaunched app activates itself (Electron apps do,
  after the endpoint is already up; `lsappinfo` needs no TCC grant). It refuses
  a port another process already serves, naming that process, because the
  endpoint poll after a relaunch would otherwise answer for the wrong app.
...
**No idle wait before input.** Every XCUICoordinate verb (`tap`, `press…thenDragTo`, `typeText`) can wait for the target app to go quiescent before the event, bounded by XCTest's 60 s `_XCTApplicationStateTimeout`; on a surface that never settles (a Home Screen icon menu, a browser new-tab page) the press may land while the verb still times out. At setup, `disableQuiescenceWait()` in `InterceptorRunnerUITests.swift` replaces available `XCUIApplicationProcess`, `XCUIApplication`, and `XCAXClient_iOS` wait selectors with no-ops. Xcode 26 uses the process `…isPreEvent:` selectors; its application `_waitForQuiescence` path calls `_waitForQuiescenceAsPreEvent:`. Missing selectors are skipped, and `ios fgdebug` lists the methods patched on the connected device. The synthesized-event lane (`ios gesture`) never waited. `ios click --x 10.5 --y 20.25` and the coordinate forms of `drag` and `scroll` keep fractions (`Number`, not `parseInt`). `ios keys Enter|Return|Tab` as the whole argument maps to the control character `typeText` presses; any other text is typed literally.

**Default input follows the foreground app.** `Runner.app()` resolves through `foregroundApp()`, the same lookup used by `tree` and `find`. After a runner restart, `ios keys`, `ios type`, coordinate input, and the on-device `Interceptor.type` therefore target the app on screen without a separate `ios app activate`. An explicitly activated bundle still wins; the lookup falls back to SpringBoard for system-owned controls. A runner refresh installs this change on an existing phone.
