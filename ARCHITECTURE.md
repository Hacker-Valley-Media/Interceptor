...
Trusted credential typing also passes sensitivity through the focus request. `handleFocus` verifies that the target received focus, including shadow descendants, and the background handler refuses delivery after a failed focus result. `daemon/os-input.ts` assigns explicit modifier flags to every text event, including zero for lowercase, digits, Unicode, and Shift release, so the events do not inherit unrelated modifiers.
