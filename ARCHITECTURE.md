...
- **Full-group purge (`closeGroupWhenDone`, opt-in)** — the guarded sweep is deliberately conservative, and in agent use that conservatism compounds: an agent that typed into any form leaves a permanently dirty tab, every vetoed pass re-stamps the group's idle clock, and the group outlives the session it belonged to. With the popup toggle on, an idle group is not swept but **deleted**: `planGroupPurge` returns every tab id in the group, all of G2–G5 and the dirty check are skipped (a timeout-is-dirty verdict on a wedged page would otherwise veto the purge), and the group object disappears with its last tab. Two properties are kept deliberately. It cannot fire without `idleCloseMinutes > 0` — idle is the only "done" signal the extension has, since the CLI is one process per command — and it will not take the browser down with the group: `chrome.tabs.remove` on a window's last tab closes the window, and on Windows/Linux the last window closing quits the browser (service worker, daemon connection, and every other lane with it), so when the purge would empty the whole profile an ungrouped blank survivor tab is created first. The profile tab count is re-read per group, because an earlier purge in the same tick makes a cached count stale-high in exactly the direction that would skip the survivor. A purged group's stamp is removed rather than refreshed, so the next group minted under that label starts from a grace stamp instead of looking instantly idle.
