...
**Browser surface:** `interceptor open <url>`, `interceptor tab new <url>`, and `interceptor window new [url]` create tabs and windows in the background by default. The user's currently-active tab stays active. When any window already holds Interceptor tab groups, new tabs are created there (the caller's own group's window first) rather than in the window the user is focused on; a new window is created only when the profile has no normal window. The only verbs that move the active tab or focused window are: `open --activate`, `tab new --activate`, `window new --activate`, `tab switch <id>`, `window focus <id>`, and `sessions restore <id> --activate`. A page that will not render while hidden is not a reason to switch: `interceptor tab keepalive <id>` makes it read as visible and run its animation frames in place. After a `tab switch` for a trusted click, `tab switch` back to the tab that was showing is allowed once even though that tab is unmanaged. Plain `sessions restore <id>` reopens the closed page as a background tab in your group; it does not bring back history or form state, and `--activate` is the browser's own full restore. The reuse path (`open --reuse`) preserves the reused tab's current focus state — call `open --reuse --activate` to also foreground it. Every other browser verb (`click`, `type`, `read`, `tree`, `text`, `inspect`, `screenshot`, `net`, `cookies`, `scroll`, etc.) operates on the target tab without disturbing the user's active tab.
...
- `has no GUI login on this Mac, so launchd has no gui/<uid> session` (from `interceptor macos *` or the `status` bridge hint) → the CLI is running as an account that is not logged in at the screen (an ssh session, or a service account). The bridge is a LaunchAgent and exists only inside that account's GUI session; `launchctl bootstrap` cannot help because the domain does not exist. Run the CLI as the user the message names as owning the screen, or log this account in at the screen.
- `daemon pid N (…) holds port P but belongs to another user` → a second macOS account's daemon from a release before per-user ports (1.0.3) owns that port. Each account now has its own runtime files (`$TMPDIR/interceptor.{sock,pid,lock}`) and port pair (uid 501 keeps 19221/19222). Log that account out or reinstall Interceptor (the installer stops every account's daemon), then retry; do not try to unlink files under `/tmp`.
...
- `eval --main` or `save` printed `warning: … the tab was reloaded with its CSP header stripped` → the page's in-memory state is gone. On a stateful page (an open conversation, a half-filled form) re-run with `eval --main --no-reload` and take the CSP error instead of the reload.
